...
#
#   2. All five files must build TOGETHER against NT8's references. `nt8c check`
...
#
# So this concatenates the five into one compilation unit — usings hoisted to
# the top, which is where C# requires them — and checks that. It is the closest
# thing to NT8's own F5 that runs from the shell.
#
...
FILES=(BreakBoxTypes BreakBoxCloud BreakBoxCore BreakBoxExits BreakBoxHistory BreakBoxStrategy BreakBoxPanel)
...
echo ""
echo "== 2/2  all five files against NT8 references"
