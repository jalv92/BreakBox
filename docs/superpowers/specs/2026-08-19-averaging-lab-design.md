...
| Daily budget | `L_arm = min(BudgetFraction × DailyLossLimit, DailyLossLimit − day loss so far)`, via the host's existing `_dayPnl`/`CheckDailyLimits` machinery; **no replenishment from wins**; one armed trade at a time | a later trade in a losing day gets a smaller grid automatically; host lockout semantics unchanged |
